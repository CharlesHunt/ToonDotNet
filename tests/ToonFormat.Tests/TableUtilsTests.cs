...
{
    #region ToList (Dictionary) Tests

    [Fact]
    public void ToList_SimpleDataTable_ReturnsListOfDictionaries()
    {
        // Arrange
    ...
    [Fact]
    public void ToList_DataTableWithDBNull_ConvertsToNull()
    {
        // Arrange
        var table = new DataTable();
        table.Columns.Add("id", typeof(int));
    ...
    [Fact]
    public void ToList_DataTableWithMixedTypes_PreservesTypes()
    {
        // Arrange
        var table = new DataTable();
        table.Columns.Add("id", typeof(int));
        ...
        table.Rows.Add(1, "Alice", 98.5, true);
        ...
        Assert.IsType<double>(result[0]["score"]);
        Assert.IsType<bool>(result[0]["active"]);
    }

    [Fact]
    public void ToList_NullDataTable_ThrowsArgumentNullException()
    {
        // Arrange
        DataTable? table = null;

        // Act & Assert
        Assert.Throws<ArgumentNullException>(() => table!.ToList());
    }

    [Fact]
    public void ToList_DataTableWithDateTime_PreservesDateTime()
    {
        // Arrange
        var table = new DataTable();
        table.Columns.Add("id", typeof(int));
        ...
        // Assert
        Assert.Single(result);
        Assert.Equal(date, result[0]["created"]);
    }

    [Fact]
    public void ToList_DataTableWithGuid_PreservesGuid()
    {
        // Arrange
        var table = new DataTable();
    ...
    [Fact]
    public void ToList_IntegratesWithToonEncode_ProducesTabularFormat()
    {
        // Arrange
        var table = new DataTable();
        table.Columns.Add("id", typeof(int));
        ...
        var toonString = Toon.Encode(list);

        // Assert
        Assert.Contains("[2]{id,name,role}:", toonString);
        Assert.Contains("1,Alice,admin", toonString);
        Assert.Contains("2,Bob,user", toonString);
    }

    [Fact]
    public void ToListTyped_RoundTrip_PreservesData()
    {
        // Arrange
        var originalTable = new DataTable();
        originalTable.Columns.Add("Id", typeof(int));
        originalTable.Columns.Add("Name", typeof(string));
        originalTable.Columns.Add("Role", typeof(string));
        originalTable.Rows.Add(1, "Alice", "admin");
        originalTable.Rows.Add(2, "Bob", "user");

        // Act
        var typedList = originalTable.ToList<TestUser>();
        var dictList = originalTable.ToList();

        // Assert - Both should produce same TOON output
        var toonFromTyped = Toon.Encode(typedList);
        var toonFromDict = Toon.Encode(dictList);
        Assert.Equal(toonFromDict, toonFromTyped);
    }
