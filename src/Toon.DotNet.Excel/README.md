# Toon.DotNet.Excel - Toon Spec Version 4.1.1
---
...
- **Object-of-objects values** (spec §9.5 keyed tabular form, e.g. `Sales[2:]{age,city}:`, now the default core encoder output) become a proper worksheet — an entry per row, with a leading `key` column holding each entry's name, rather than a raw JSON blob in a single cell
...
Decoding reverses the mapping: a root TOON object creates one sheet per key; a root array creates a single sheet named `Sheet1`. A per-key value that is itself an object-of-objects (spec §9.5 keyed tabular form) becomes a keyed table — an entry per row, with a leading `key` column — rather than raw JSON text in a single cell.
...
### Decode a keyed tabular dataset to a proper worksheet

```csharp
// Sales[2:]{age,city}: (spec §9.5 keyed tabular form) decodes to a
// "key" column plus one column per field, not a raw JSON blob.
string toon = "Sales[2:]{age,city}:\n  alice: 30,Berlin\n  bob: 25,Oslo";
using var wb = toon.ToExcelWorkbook();
var sheet = wb.Worksheet("Sales");
// sheet.Cell(1,1) == "key", sheet.Cell(2,1) == "alice", sheet.Cell(2,2) == 30
```
...
| `SpecVersion` | `ToonSpecVersion.V4` | Controls nested field groups, keyed tabular form, and leading-plus numeric-like string quoting on the encode side |
...
| `LegacyCompatibility` | `false` | Opt back into pre-v4 lenient decoding for token trimming scope and misplaced-scalar tolerance |
...
| Object-of-objects (keyed tabular) | — | A worksheet with a `key` column plus one column per field |
| Other nested object/array cell values | Raw JSON text | Raw JSON text |
